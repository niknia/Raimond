using System.Linq;
using Dkd.Infra.Redis.Caching.Core.Interceptor;
    ...
    : AbstractAppService, ISysConfigService
{
    public async Task<ServiceResult<IdDto>> CreateAsync(SysConfigCreationDto input)
    {
        ...
        if (keyExists)
        {
            return Problem(HttpStatusCode.BadRequest, "Configuration key already exists");
        }

        var nameExists = await sysConfigRepo.AnyAsync(x => x.Name == input.Name);
        if (nameExists)
        {
            return Problem(HttpStatusCode.BadRequest, "Configuration name already exists");
        }
    ...
    public async Task<ServiceResult> UpdateAsync(long id, SysConfigUpdationDto input)
    {
        ...
        if (entity is null)
        {
            return Problem(HttpStatusCode.NotFound, "Configuration does not exist");
        }
    ...
    public async Task<ServiceResult> DeleteByKeyAsync([CachingParam] string ids)
    {
        await sysConfigRepo.ExecuteDeleteAsync(x => x.Key.Equals(ids));
        return ServiceResult();
    }
    ...
    public async Task<List<SysConfigDto>> GetListbykeyAsync(string keys)
    {
        var entity = await sysConfigRepo.Where(x => x.Key.Contains(keys)).ToListAsync();
        return entity is null ? new List<SysConfigDto>() : Mapper.Map<List<SysConfigDto>>(entity);
    }
