...
public sealed class DeepLTranslationService(
    ...
    IOptions<DeepLTranslationOptions> options) : ITranslationService
{
    ...
    public async Task<IReadOnlyList<TranslationResult>> TranslateAsync(IEnumerable<string> texts, string targetLanguage,
        ...
        CancellationToken ct = default)
    {
        ...
        var parameters = new List<KeyValuePair<string, string>> { new("target_lang", GetLanguageCode(targetLanguage)) };
        ...
        var url =
            options.AuthKey.EndsWith(":fx", StringComparison.Ordinal) ? UrlFree : UrlPaid;
    ...
    {
        [JsonPropertyName("translations")] public TranslationDto[] Translations { get; set; }
    }
