...
public static class SvgExtensions
{
    public static readonly HashSet<string>
        AllowedUriSchemes = new(StringComparer.OrdinalIgnoreCase) { "http", "https" };
    ...
    public static SvgMetadata GetSvgMetadata(this string html)
    {
        ...
        while (reader.Read())
        {
            if (reader.TokenKind != HtmlTokenKind.Tag ||
                !reader.NameAsMemory.Span.Equals("svg", StringComparison.OrdinalIgnoreCase))
            {
                continue;
            }
            ...
            for (var i = 0; i < reader.AttributeCount; i++)
            {
                ...
                else if (SvgAttributes.Urls.Contains(attributeName))
                {
                    ...
                    else
                    {
                        ...
                        {
                            errors.Add(new SvgError(
                                $"Invalid URL scheme '{uri.Scheme}' for attribute '{attributeName}'",
                                reader.LineNumber,
                                reader.LinePosition));
                        }
                    }
