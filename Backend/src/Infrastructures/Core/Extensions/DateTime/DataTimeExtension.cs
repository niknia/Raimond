...
public static class DataTimeExtension
{
    ...
    public static int GetWeekAmount(this DateTime _, int year)
    {
        ...
        return gc.GetWeekOfYear(end, CalendarWeekRule.FirstDay, DayOfWeek.Monday); //Number of weeks in the year
    }
    ...
    {
        //Get the year part of the input parameter to determine if it's a leap year
        var n = value.Year;
    ...
    public static string DateDiff(this in DateTime dateTime1, in DateTime dateTime2)
    {
        ...
        if (ts.Days >= 1)
        {
            dateDiff = dateTime1.Month + " month " + dateTime1.Day + " day";
        }
        else
        {
            dateDiff = ts.Hours > 1 ? ts.Hours + " hours ago" : ts.Minutes + " minutes ago";
        }
    ...
    public static string GetDiffTime(this in DateTime beginTime, in DateTime endTime)
    {
        ...
        if (sec > year)
        {
            strResout += sec / year + " year";
            sec %= year; // Remaining
        }

        if (sec > month)
        {
            strResout += sec / month + " month";
            sec %= month;
        }

        if (sec > day)
        {
            strResout += sec / day + " day";
            sec %= day;
        }

        if (sec > hours)
        {
            strResout += sec / hours + " hour";
            sec %= hours;
        }

        if (sec > minutes)
        {
            strResout += sec / minutes + " minute";
            sec %= minutes;
        }

        strResout += sec + " second";
        return strResout;
    }
