using Dkd.Infra.Core.Configuration;
...
public sealed class FlowOptions : IValidatableOptions
{
    ...
    public IEnumerable<ConfigurationError> Validate()
    {
        ...
        {
            yield return new ConfigurationError($"Value must be between 0 and {NumWorker - 1} (NumWorker).",
                nameof(WorkerIndex));
        }

        if (NumPartitions % NumWorker != 0)
        {
            yield return new ConfigurationError("Value must be a multiple of the number of workers.",
