    ...
    : IFlowExecutor<TContext> where TContext : FlowContext
{
        ...
        CancellationToken ct)
    {
        ...
        foreach (var (stepId, stepDefinition) in definition.Steps)
        {
            ...
            await ValidateCoreAsync(stepDefinition.Step, context,
                (path, type, message) => { addError($"Steps.{stepId}.Step.{path}", type, message); }, ct);
        }
    }
    ...
    public FlowExecutionState<TContext> CreateState(CreateFlowInstanceRequest<TContext> request)
    {
        ...
        if (!request.Definition.Steps.ContainsKey(request.Definition.InitialStepId.Value))
        {
            throw new InvalidOperationException(
                $"Flow definition has no step with ID '{request.Definition.InitialStepId}'.");
        }
    ...
    private static async Task ValidateCoreAsync(FlowStep step, FlowValidationContext validationContext,
        AddError addError,
        CancellationToken ct)
    {
        ...
        await step.ValidateAsync(validationContext,
            (path, message) => { addError(path, ValidationErrorType.InvalidProperty, message); }, ct);
    }
    ...
    {
        return (executionContext, ct) =>
        {
            NextStepDelegate next = () => { return executionContext.Step.ExecuteAsync(executionContext, ct); };

            foreach (var middleware in middlewares.Reverse())
            {
                var currentNext = next;
                next = () => { return middleware.InvokeAsync(executionContext, currentNext, ct); };
            }
