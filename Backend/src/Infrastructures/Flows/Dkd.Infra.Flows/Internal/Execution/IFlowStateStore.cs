...
public interface IFlowStateStore<TContext> where TContext : FlowContext
{
    ...
    IAsyncEnumerable<FlowExecutionState<TContext>> QueryPendingAsync(int[] partitions, Instant now,
        CancellationToken ct = default);

    Task<(List<FlowExecutionState<TContext>> Items, long Total)> QueryByOwnerAsync(string ownerId,
